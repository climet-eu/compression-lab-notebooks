...
def _replace_reference_path(ref: dict, grib_path: Path, remote_url: str | None) -> dict:
...
def generate_ref(
    ...
    try:
        ...
        stem = (
            output_path.name[: -len(".ref")]
            if output_path.name.endswith(".ref")
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate gribscan .ref metadata for an ERA5 GRIB file."
    )
    ...
    if args.local_paths:
        ...
        remote_url = (
            args.remote_url or f"{args.base_url.rstrip('/')}/{args.grib_path.name}"
        )
